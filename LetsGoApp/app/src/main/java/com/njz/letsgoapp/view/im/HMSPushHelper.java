        ...
        try {
            if(Class.forName("com.huawei.hms.support.api.push.HuaweiPush") != null){
                ...
                if(!TextUtils.isEmpty(buildVersion)){
                    LogUtil.e("huawei hms push is available!");
                    isUseHMSPush = true;
                    HMSAgent.init(application);
                }else{
                    LogUtil.e("huawei hms push is unavailable!");
                }
            }else{
                LogUtil.e("no huawei hms push sdk or mobile is not a huawei phone");
            }
        } catch (Exception e) {
            LogUtil.e("no huawei hms push sdk or mobile is not a huawei phone");
        }
        ...
        if (isUseHMSPush) {
            HMSAgent.connect(activity, new ConnectHandler() {
                ...
                public void onConnect(int rst) {
                    LogUtil.e("huawei hms push connect result code:" + rst);
                    if (rst == HMSAgent.AgentResultCode.HMSAGENT_SUCCESS) {
                        HMSAgent.Push.getToken(new GetTokenHandler() {
                            @Override
                            public void onResult(int rst) {
                                LogUtil.e("get huawei hms push token result code:" + rst);
                            }
                        });
                    }
                }
